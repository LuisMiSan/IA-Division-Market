        ...
        <div className="relative w-full max-w-[170px] aspect-[9/18] bg-gray-900 rounded-[1.25rem] border-[4px] border-gray-800 shadow-xl overflow-hidden mb-3 transition-all duration-300 ease-out group-hover:scale-105 group-hover:shadow-2xl group-hover:shadow-blue-500/30">
            ...
            {onEdit && (
              <button 
                ...
                className="absolute top-2 right-2 z-50 p-1.5 bg-black/60 hover:bg-blue-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-200 transform hover:scale-110"
                title="Editar App"
              >
            ...
            <div className="absolute inset-0 bg-slate-900 flex flex-col cursor-default select-none">
                ...
                {app.coverUrl && app.coverUrl.trim() !== '' ? (
                    // Full Cover Image Mode
                    <div className="absolute inset-0 top-4 bottom-0 bg-slate-900">
                         <img src={app.coverUrl} alt={`${app.name} cover`} className="w-full h-full object-cover" />
                         {/* Optional gradient overlay for text readability at bottom */}
